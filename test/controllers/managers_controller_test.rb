...
class ManagersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create manager" do
    assert_difference('Manager.count') do
      post managers_url, params: { manager: { description: @manager.description, email: @manager.email, name: @manager.name } }
    end
  ...
  test "should update manager" do
    patch manager_url(@manager), params: { manager: { description: @manager.description, email: @manager.email, name: @manager.name } }
